...
class TrajectoryIO:
    ...
    def load_tum_trajectory(
        filename: Path,
    ) -> Tuple[Float[Tensor, "num_poses"], Float[LieTensor, "num_poses 7"]]:
    ...
    def write_tum_trajectory(
        ...
        with open(filename, "w", encoding="UTF-8") as f:
            ...
            for timestamp, pose in zip(timestamps, poses):
                f.write(
                    f"{timestamp.item()} {pose[0]} {pose[1]} {pose[2]} {pose[3]} {pose[4]} {pose[5]} {pose[6]}\n"
                )

    @staticmethod
    def write_kitti_trajectory(
        filename: Path, poses: Float[LieTensor, "num_poses 7"] | Float[Tensor, "num_poses 7"]
    ):
