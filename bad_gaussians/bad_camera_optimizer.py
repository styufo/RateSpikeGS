...
class BadCameraOptimizerConfig(CameraOptimizerConfig):
    ...
    """Second-order smoothness penalty on SE(3) control knots in tangent space."""
...
class BadCameraOptimizer(CameraOptimizer):
    ...
    def forward(
        ...
        if (
            ...
            if self.non_trainable_camera_indices.device != self.pose_adjustment.device:
                self.non_trainable_camera_indices = self.non_trainable_camera_indices.to(
                    self.pose_adjustment.device
                )
        ...
        if mode == "uniform":
            ...
            if self.config.mode == "linear":
                ...
                return cubic_bspline_interpolation(
                    camera_opt, torch.tensor([0.5], device=camera_opt.device)
                ).squeeze(1)
            elif self.config.mode == "bezier":
            ...
            if self.config.mode == "linear":
                ...
                return cubic_bspline_interpolation(
                    camera_opt, torch.tensor([0.0], device=camera_opt.device)
                ).squeeze(1)
            elif self.config.mode == "bezier":
                return bezier_interpolation(
                    camera_opt, torch.tensor([0.0], device=camera_opt.device)
            ...
            if self.config.mode == "linear":
                ...
                return cubic_bspline_interpolation(
                    camera_opt, torch.tensor([1.0], device=camera_opt.device)
                ).squeeze(1)
            elif self.config.mode == "bezier":
                return bezier_interpolation(
                    camera_opt, torch.tensor([1.0], device=camera_opt.device)
    ...
    def apply_to_raybundle(self, *args, **kwargs):
        ...
        raise NotImplementedError(
            "Not implemented in BAD-Gaussians. Please checkout https://github.com/WU-CVGL/Bad-RFs"
        )
    ...
    def apply_to_camera(self, camera: Cameras, mode: TrajSamplingMode) -> List[Cameras]:
        ...
        if (
            ...
            or camera.metadata is None
            or not ("cam_idx" in camera.metadata)
        ):
        ...
        if c2w.shape[1] == 3:
            c2w = torch.cat(
                [c2w, torch.tensor([0, 0, 0, 1], device=c2w.device).view(1, 1, 4)], dim=1
            )
        ...
        if mode == "uniform":
            c2ws = c2w.tile(
                (self.config.num_virtual_views, 1, 1)
            )  # shape: (num_virtual_views, 4, 4)
    ...
    def get_loss_dict(self, loss_dict: dict) -> None:
        ...
        if self.config.mode == "off":
            return
        if self.config.trans_l2_penalty > 0.0:
            loss_dict["camera_translation_reg"] = (
                self.config.trans_l2_penalty * self.pose_adjustment[..., :3].square().mean()
            )
        if self.config.rot_l2_penalty > 0.0:
            loss_dict["camera_rotation_reg"] = (
                self.config.rot_l2_penalty * self.pose_adjustment[..., 3:].square().mean()
            )
        if self.config.trajectory_smoothness_weight > 0.0 and self.num_control_knots >= 3:
            second_difference = (
                self.pose_adjustment[:, 2:]
                - 2.0 * self.pose_adjustment[:, 1:-1]
                + self.pose_adjustment[:, :-2]
            )
            loss_dict["camera_trajectory_smoothness"] = (
                self.config.trajectory_smoothness_weight * second_difference.square().mean()
            )
